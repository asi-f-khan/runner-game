...
function hascrashed(obstacle) {
  ...
  if (
    piecebottom < obstacletop ||
    piecetop > obstaclebottom ||
    pieceright < obstacleleft ||
    pieceleft > obstacleright
  )
    return false;
  else return true;
}
...
function updateGameArea() {
  ...
  if (frameno == 0 || everyinterval(obstacles[obstacles.length - 1])) {
    ...
    obstacles.push(
      new component(10, y - height - gap, "white", x, height + gap)
    );
  }
